...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/12 23:26:55 by d2435             #+#    #+#             */
/*   Updated: 2021/02/12 23:26:57 by d2435            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_player_dir(t_system *sys, char dir)
{
	...
	{
		sys->player.dir_y = -1;
		sys->player.plane_x = (((double)sys->cub.res_x /
			(double)sys->cub.res_y) - 1.0);
	}
	else if (dir == 'S')
	{
		sys->player.dir_y = 1;
	...
	{
		sys->player.dir_x = 1;
		sys->player.plane_y = (((double)sys->cub.res_x /
			(double)sys->cub.res_y) - 1.0);
	}
	else if (dir == 'W')
	{
...
void	ft_set_player_start(t_system *sys, int x, int y)
{
	...
	if (sys->player.plane_x > 0)
		sys->player.plane_x = (sys->player.plane_x < 0.5 ? 1 -
			sys->player.plane_x : sys->player.plane_x);
	if (sys->player.plane_y > 0)
		sys->player.plane_y = (sys->player.plane_y < 0.5 ? 1 -
			sys->player.plane_y : sys->player.plane_y);
	sys->cub.map[y][x] = '0';
