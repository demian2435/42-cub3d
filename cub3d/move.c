...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/12 23:27:14 by d2435             #+#    #+#             */
/*   Updated: 2021/02/14 10:00:29 by dmalori          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_rotate_left(t_system *sys)
{
	...
	sys->player.dir_x = (sys->player.dir_x * cos(-0.02)) -
		(sys->player.dir_y * sin(-0.02));
	sys->player.dir_y = (olddirx * sin(-0.02)) + (sys->player.dir_y *
		cos(-0.02));
	oldplanex = sys->player.plane_x;
	sys->player.plane_x = (sys->player.plane_x * cos(-0.02)) -
		(sys->player.plane_y * sin(-0.02));
	sys->player.plane_y = (oldplanex * sin(-0.02)) + (sys->player.plane_y *
		cos(-0.02));
}
...
void	ft_rotate_right(t_system *sys)
{
	...
	sys->player.plane_x = (sys->player.plane_x * cos(0.02)) -
		(sys->player.plane_y * sin(0.02));
	sys->player.plane_y = (oldplanex * sin(0.02)) +
		(sys->player.plane_y * cos(0.02));
}
...
void	ft_move_up_down(t_system *sys)
{
	...
	{
		new_x = sys->player.pos_x + sys->player.dir_x * sys->player.speed;
		new_y = sys->player.pos_y + sys->player.dir_y * sys->player.speed;
